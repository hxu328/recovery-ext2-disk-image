					...
					)
{
		...
		off_t offset = BASE_OFFSET + BLOCK_OFFSET(blocks_per_group * ngroup) + block_size;
		printf("Offset is: %li\n", offset);
     ...
     struct ext2_inode             *inode;     /* where to put the inode */
{
		...
		// off_t offset_inode = BLOCK_OFFSET(blocks_per_group * ngroup) + offset + (inode_no-1)*sizeof(struct ext2_inode);
		// printf("Read inode: %d; offset: %li\n", inode_no, offset_inode);
}
